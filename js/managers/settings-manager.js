...
class SettingsManager {
    ...
    renderRandomTablesTab() {
        const tabContent = document.getElementById('random-tables-tab');
        if (!tabContent) {
            console.error('❌ random-tables-tab not found in settings');
            return;
        }

        // Réutiliser directement le contenu de l'AdventureManager
        if (window.adventureManager) {
            // Sauvegarder temporairement le contenu de l'onglet de l'aventure
            const adventureTabContent = document.getElementById('random-tables-content');
            if (adventureTabContent) {
                // Cloner le contenu HTML
                tabContent.innerHTML = adventureTabContent.innerHTML;
            } else {
                // Fallback : générer le contenu nous-mêmes
                ...
                if (uploadInput) {
                    uploadInput.onchange = (e) => window.adventureManager.handleRandomTableUpload(e);
                }
            }
        }
    }
