...
class CharactersManager {
  ...
  setupEventListeners() {
    ...
    if (deleteAllBtn) {
      deleteAllBtn.addEventListener("click", () =>
        this.deleteAllCharactersForActiveMap(),
      );
    }
    ...
    if (importFileInput) {
      ...
      const newImportFileInput = document.getElementById(
        "import-characters-input",
      );
      newImportFileInput.addEventListener("change", (event) =>
        this.handleImportCharacters(event),
      );
    }
    ...
    const filterKnownLocations = document.getElementById(
    ...
    try {
      ...
      if (stored) {
        ...
        this.characters = this.characters.map((char) => ({
          ...
          associatedLocations: (char.associatedLocations || []).map((id) =>
            String(id),
          ),
          associatedRegions: (char.associatedRegions || []).map((id) =>
            String(id),
          ),
        }));

        console.log(
          `📚 [CharactersManager.loadCharactersFromLocal] ${this.characters.length} personnages chargés depuis localStorage.`,
        );
        this.characters.forEach((c) => {
          console.log(
            `   - Character: "${c.name}" | ID: ${c.id} | Origin: ${c.origin} | Campaign: ${c.campaignName || "N/A"}`,
          );
        });
  ...
  openCharactersModal() {
    ...
    if (modal) {
      ...
      const isAdventureActive =
        ...
        typeof window.positionManager.isAdventureActive === "function"
          ? window.positionManager.isAdventureActive()
          : window.positionManager?.adventureMode === "mj" ||
      ...
      if (isAdventureActive) {
        ...
        const filterContainer = modal.querySelector(
  ...
  syncFiltersUI() {
    ...
    if (filterKnownLocations)
      filterKnownLocations.checked = this.filters.knownLocations;
  ...
  renderCharactersList() {
    ...
    console.log(
      `👥 [CharactersManager.renderCharactersList] Rendering list for active campaign ID: ${activeCampaignId || "NONE"}`,
    );
    ...
    let filteredCharacters = this.characters.filter((character) => {
      ...
      if (
        ...
        String(character.campaignId) !== String(activeCampaignId)
      ) {
        return false;
      }
    ...
    console.log(
      `👥 [CharactersManager.renderCharactersList] ${filteredCharacters.length}/${this.characters.length} personnages passent le filtre campagne/carte.`,
    );
    filteredCharacters.forEach((c) => {
      console.log(
        `   - Rendering Character: "${c.name}" | ID: ${c.id} | Origin: ${c.origin} | Campaign: ${c.campaignName} (${c.campaignId})`,
      );
    });
    ...
    filteredCharacters.forEach((character) => {
      const hasLocations =
        ...
        character.associatedLocations.length > 0;
      const hasRegions =
        character.associatedRegions && character.associatedRegions.length > 0;
  ...
  renderCharacterCard(character) {
    ...
    if (
      displayImage &&
      displayImage.url &&
      displayImage.url.includes("/thumbs/")
    ) {
    ...
    const explicitVignette = character.images?.find(
      (img) => img.type === "vignette",
    );
  ...
  getAssociatedRegionNames(character) {
    if (
      ...
      character.associatedRegions.length === 0
    ) {
      return [];
    }
  ...
  openAddCharacterModal() {
    ...
    if (modal) {
      ...
      const uploadContainer = document.getElementById(
        "character-image-upload-container",
      );
      if (uploadContainer && window.uploadManager) {
        uploadContainer.innerHTML = "";
        window.uploadManager.createImageSelector(
          uploadContainer,
          "locations",
          (result) => {
            if (result) {
  ...
  updateImagesList() {
    ...
    if (!this.tempCharacterImages || this.tempCharacterImages.length === 0) {
      listContainer.innerHTML =
        '<p class="text-gray-500 text-sm">Aucune image ajoutée</p>';
      return;
    }

    const html = this.tempCharacterImages
      .map(
        (image, index) => `
  ...
  toggleImageTypeMenu(index) {
    ...
    if (menu) {
      document
        .querySelectorAll('[id^="char-image-type-menu-"]')
        .forEach((m) => {
  ...
  setImageType(index, type) {
    if (
      !this.tempCharacterImages ||
      index < 0 ||
      index >= this.tempCharacterImages.length
    ) {
      return;
    }
  ...
  async generateCharacterDescription() {
    ...
    try {
      ...
      const characterType = document.querySelector(
        'input[name="character-type"]:checked',
      ).value;
      const characterName = document
        .getElementById("character-name-input")
        .value.trim();
      const characterDescriptionInput = document
        .getElementById("character-desc-input")
        .value.trim(); // Description pré-existante
      const activeMapUrl =
        window.settingsManager?.activeMapUrl ||
        "fr_tor_2nd_eriadors_map_page-0001.webp";
      const activeMapName =
        window.settingsManager?.activeMapName || "Carte inconnue";

      // Obtenir les détails de la carte active (région, lieu)
      const currentPos = window.positionManager?.currentPosition || {
        x: 0,
        y: 0,
      }; // Position du marqueur de position
      ...
      const generatedDescription = await window.geminiManager.generateContent(
        prompt,
        null,
      ...
      if (generatedDescription && typeof generatedDescription === "string") {
        ...
        this.showNotification(
          "Description générée",
          "La description du personnage a été générée par Gemini.",
          "success",
        );
        ...
        this.showNotification(
          ...
          "Impossible de générer la description avec Gemini. Réponse invalide.",
          "error",
        );
      }
    } catch (error) {
  ...
  showCharacterInfoBox(characterId) {
    ...
    console.log(
      `👁️ [showCharacterInfoBox] Affichage de "${character.name}" avec:`,
      {
    ...
    if (window.infoBoxManager) {
      ...
      setTimeout(() => {
        console.log(
          `📋 [showCharacterInfoBox] Affichage forcé de l'onglet Description pour ${character.name}`,
        );
  ...
  exportCharacters() {
    try {
      ...
      console.log(
        `📤 [exportCharacters] Total personnages disponibles: ${this.characters?.length || 0}`,
      );
      ...
      console.log(
        `✅ Export terminé - ${charactersToExport.length} personnages de "${activeMapName}" sauvegardés`,
      );
      this.showNotification(
        "Export réussi",
        `${charactersToExport.length} personnages de "${activeMapName}" exportés`,
        "success",
      );
    } catch (error) {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (
          ...
          this.showNotification(
            "Import échoué",
            "Format de fichier invalide. Attendu: { characters: [...] }",
            "error",
          );
        ...
        this.showNotification(
          "Erreur d'import",
          "Fichier JSON invalide: " + err.message,
          "error",
        );
        this.isImporting = false;
      }
  ...
  processImportCharacters(importedCharacters, mode) {
    try {
      if (mode === "replace") {
        ...
        importedCharacters.forEach((importedChar) => {
          ...
          const normalizedImportedChar = {
            ...
            associatedLocations: (importedChar.associatedLocations || []).map(
              (id) => String(id),
            ),
            associatedRegions: (importedChar.associatedRegions || []).map(
              (id) => String(id),
            ),
            Rumeurs:
              importedChar.Rumeurs ||
              (importedChar.Rumeur ? [importedChar.Rumeur] : []),
          };
          // Supprimer mapId si présent
          delete normalizedImportedChar.mapId;

          const existingChar = this.characters.find(
            (c) => String(c.id) === normalizedImportedChar.id,
          );
          ...
          if (existingChar) {
            ...
            Object.assign(existingChar, {
              description:
                normalizedImportedChar.description || existingChar.description,
              type: normalizedImportedChar.type || existingChar.type,
              images: normalizedImportedChar.images || existingChar.images,
              associatedLocations:
                normalizedImportedChar.associatedLocations ||
                existingChar.associatedLocations,
              associatedRegions:
                normalizedImportedChar.associatedRegions ||
                existingChar.associatedRegions,
              Rumeurs: normalizedImportedChar.Rumeurs || existingChar.Rumeurs,
              campaignId: window.authManager?.currentCampaignId,
              campaignName: window.authManager?.currentCampaignName,
            });
            console.log(
              `🔄 Personnage mis à jour: ${normalizedImportedChar.name} (ID: ${normalizedImportedChar.id})`,
            );
          } else {
            ...
            console.log(
              `➕ Nouveau personnage ajouté: ${normalizedImportedChar.name}`,
            );
          }
        });
      }
      ...
      this.showNotification(
        "Import réussi",
        `${importedCharacters.length} personnage(s) importé(s) (mode: ${mode})`,
        "success",
      );
      console.log(
        `✅ Import terminé: ${importedCharacters.length} personnages`,
      );
  ...
  syncCharacterLocationsFromLocationData() {
    if (!window.locationsData || !window.locationsData.locations) {
      console.warn(
        "⚠️ Pas de données de localisation disponibles pour la synchronisation bidirectionnelle.",
      );
      return;
    }
      ...
      associatedCharacterIds.forEach((charId) => {
        ...
        if (character) {
          ...
          if (!character.associatedLocations.includes(locationId)) {
            ...
            console.log(
              `➕ Lieu "${location.name}" ajouté à "${character.name}"`,
            );
          }
        } else {
          ...
          console.log(
            `ℹ️ Personnage ${charId} associé au lieu "${location.name}" mais non trouvé dans la liste des personnages.`,
          );
        }
      });
    });
    ...
    this.characters.forEach((character) => {
      ...
      associatedLocationIds.forEach((locationId) => {
        const location = window.locationsData.locations.find(
          (loc) => String(loc.id) === String(locationId),
        );
        if (location) {
          ...
          if (!location.associatedCharacters.includes(characterId)) {
            ...
            console.log(
              `➕ Personnage "${character.name}" ajouté au lieu "${location.name}"`,
            );
          ...
          console.warn(
            `⚠️ Lieu ${locationId} associé au personnage "${character.name}" mais non trouvé dans les données de lieux.`,
          );
        }
      });
    });
    ...
    this.saveCharactersToLocal();
    console.log(
      "🔄 Synchronisation bidirectionnelle Lieux ↔ Personnages terminée.",
    );
  }
  ...
  deleteAllCharactersForActiveMap() {
    ...
    const activeMapName =
      window.settingsManager?.activeMapName || "cette carte";
    ...
    if (charactersToDelete.length === 0) {
      this.showNotification(
        "Information",
        "Aucun personnage associé exclusivement à cette carte.",
        "info",
      );
      return;
    }
    ...
    try {
      console.log(
        `🗑️ Suppression de personnages exclusifs à la carte: ${activeMapId}`,
      );
      ...
      this.showNotification(
        "Erreur",
        "Impossible de supprimer les personnages: " + error.message,
        "error",
      );
    }
  }
  ...
  loadCharacters(
    ...
    if (data && data.characters && Array.isArray(data.characters)) {
      ...
      console.log(
        `👥 [CharactersManager.loadCharacters] ${this.characters.length} personnages chargés (Origine: ${origin}, Campagne: ${campaignName || "N/A"})`,
      );
      this.characters.forEach((c) => {
        console.log(
          `   - Cloud Character: "${c.name}" | ID: ${c.id} | Campaign: ${c.campaignName} (${c.campaignId})`,
        );
      });
      ...
      console.warn(
        "⚠️ [CharactersManager.loadCharacters] Données de personnages invalides reçues",
      );
    }
  }
  ...
  addCharacter(characterData) {
    ...
    const newCharacter = {
      ...
      associatedLocations: (characterData.associatedLocations || []).map((id) =>
        String(id),
      ),
      associatedRegions: (characterData.associatedRegions || []).map((id) =>
        String(id),
      ),
      // Pas de mapId
    };
