...
class InfoBoxManager {
    ...
    renderPersonnagesTabEdit() {
        ...
        const editForm = document.createElement('div');
        editForm.className = 'edit-form p-4';
        personnagesTab.appendChild(editForm);

        // Récupérer tous les personnages disponibles
        const allCharacters = window.charactersManager ? window.charactersManager.characters : [];
        
        // Récupérer les IDs des personnages associés (normalisés)
        const associatedCharacterIds = (this.currentItem.associatedCharacters || []).map(id => String(id));

        console.log(`🔍 [renderPersonnagesTabEdit] ${allCharacters.length} personnages disponibles`);
        console.log(`🔍 [renderPersonnagesTabEdit] IDs associés:`, associatedCharacterIds);
        ...
        if (allCharacters.length === 0) {
            editForm.innerHTML = `
                <div class="text-gray-400 text-center py-8">
                    ...
                    <p class="text-sm mt-2">Créez des personnages depuis le menu Personnages</p>
                </div>
        ...
        // Générer les checkboxes pour tous les personnages
        const checkboxesHTML = allCharacters.map((character) => {
            const charId = String(character.id);
            const isChecked = associatedCharacterIds.includes(charId);
            
            return `
                <label class="flex items-center p-2 rounded hover:bg-gray-100 cursor-pointer transition-colors" style="background-color: white;">
                    <input type="checkbox" 
                           class="character-checkbox mr-3 w-4 h-4" 
                           value="${charId}"
                           data-character-id="${charId}"
                           ${isChecked ? 'checked' : ''}>
                    <span class="text-sm" style="color: black;">${character.name}</span>
                </label>
            `;
        }).join('');
        ...
        editForm.innerHTML = `
            <div class="mb-4">
                <label class="block text-sm font-medium text-white mb-2">Personnages associés :</label>
    ...
    renderLieuxRegionsTabEdit() {
        ...
        const editForm = document.createElement('div');
        editForm.className = 'edit-form p-4';
        lieuxRegionsTab.appendChild(editForm);

        // Récupérer tous les lieux et régions disponibles
        const allLocations = window.locationsData ? window.locationsData.locations : [];
        const allRegions = window.regionsData ? window.regionsData.regions : [];
        
        // Récupérer les IDs des lieux/régions associés (normalisés)
        const associatedLocationIds = (this.currentItem.associatedLocations || []).map(id => String(id));
        const associatedRegionIds = (this.currentItem.associatedRegions || []).map(id => String(id));

        console.log(`🔍 [renderLieuxRegionsTabEdit] ${allLocations.length} lieux, ${allRegions.length} régions disponibles`);
        console.log(`🔍 [renderLieuxRegionsTabEdit] IDs lieux associés:`, associatedLocationIds);
        console.log(`🔍 [renderLieuxRegionsTabEdit] IDs régions associées:`, associatedRegionIds);

        if (allLocations.length === 0 && allRegions.length === 0) {
            editForm.innerHTML = `
                <div class="text-gray-400 text-center py-8">
                    <i class="fas fa-map-marker-alt fa-2x mb-2"></i>
                    <p>Aucun lieu ou région disponible</p>
                </div>
                <div class="flex space-x-2 mt-4">
                    <button onclick="window.infoBoxManager.saveEdit()" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded">
                        <i class="fas fa-save mr-1"></i>Sauvegarder
        ...
        const locationsHTML = allLocations.length > 0 ? `
            <h4 class="text-sm font-semibold text-white mb-2">Lieux :</h4>
            <div class="space-y-2 mb-4">
                ${allLocations.map((location) => {
                    const locId = String(location.id);
                    const isChecked = associatedLocationIds.includes(locId);
                    
                    return `
                        <label class="flex items-center p-2 rounded hover:bg-gray-100 cursor-pointer transition-colors" style="background-color: white;">
                            <input type="checkbox" 
                                   class="location-checkbox mr-3 w-4 h-4" 
                                   value="${locId}"
                                   data-location-id="${locId}"
                                   ${isChecked ? 'checked' : ''}>
                            <span class="text-sm" style="color: black;">${location.name}</span>
                        </label>
                    `;
                }).join('')}
            </div>
        ...
        // Générer les checkboxes pour les régions
        const regionsHTML = allRegions.length > 0 ? `
            <h4 class="text-sm font-semibold text-white mb-2">Régions :</h4>
            <div class="space-y-2">
                ${allRegions.map((region) => {
                    const regId = String(region.id);
                    const isChecked = associatedRegionIds.includes(regId);
                    
                    return `
                        <label class="flex items-center p-2 rounded hover:bg-gray-100 cursor-pointer transition-colors" style="background-color: white;">
                            <input type="checkbox" 
                                   class="region-checkbox mr-3 w-4 h-4" 
                                   value="${regId}"
                                   data-region-id="${regId}"
                                   ${isChecked ? 'checked' : ''}>
                            <span class="text-sm" style="color: black;">${region.name}</span>
                        </label>
                    `;
                }).join('')}
            </div>
