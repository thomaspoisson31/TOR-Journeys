...
class InfoBoxManager {
    ...
    renderLieuxRegionsTabRead() {
        console.log('📋 [renderLieuxRegionsTabRead] Début du rendu');
        
        const lieuxRegionsTab = document.getElementById('lieux-regions-tab');
        if (!lieuxRegionsTab) {
            console.warn('⚠️ Onglet lieux-regions-tab non trouvé');
            return;
        }

        if (this.currentType !== 'character') {
            console.warn('⚠️ Type incorrect, attendu character, reçu:', this.currentType);
            return;
        }

        // Recharger le personnage depuis window.charactersManager pour avoir les données à jour
        const character = window.charactersManager?.characters.find(c => String(c.id) === String(this.currentItem.id));
        if (!character) {
            console.warn('⚠️ Personnage non trouvé avec ID:', this.currentItem.id);
            return;
        }

        console.log('📋 [renderLieuxRegionsTabRead] Personnage rechargé:', character.name, {
        ...
        const textView = document.createElement('div');
        textView.className = 'text-view p-4 h-full overflow-y-auto';
        lieuxRegionsTab.appendChild(textView);

        let html = '';

        // Section Lieux associés
        const locationIds = character.associatedLocations || [];
        console.log('📋 [renderLieuxRegionsTabRead] IDs lieux associés:', locationIds);
        ...
        if (locationIds.length > 0) {
            ...
            locationIds.forEach(locationId => {
                ...
                console.log(`📋 [renderLieuxRegionsTabRead] Recherche lieu ID ${locationId}:`, location ? location.name : 'NON TROUVÉ');
                
                if (location) {
                    html += `
                        <div class="flex items-center justify-between p-3 bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors cursor-pointer"
                             onclick="window.infoBoxManager.navigateToLocation('${location.id}')">
                            <div class="flex items-center space-x-3">
        ...
        const regionIds = character.associatedRegions || [];
        console.log('📋 [renderLieuxRegionsTabRead] IDs régions associées:', regionIds);
        ...
        if (regionIds.length > 0) {
            ...
            regionIds.forEach(regionId => {
                ...
                console.log(`📋 [renderLieuxRegionsTabRead] Recherche région ID ${regionId}:`, region ? region.name : 'NON TROUVÉ');
                
                if (region) {
                    html += `
                        <div class="flex items-center justify-between p-3 bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors cursor-pointer"
                             onclick="window.infoBoxManager.navigateToRegion('${region.id}')">
                            <div class="flex items-center space-x-3">
                                <i class="fas fa-mountain text-orange-400"></i>
        ...
        if (locationIds.length === 0 && regionIds.length === 0) {
            html = '<p class="text-gray-400 italic text-center py-8">Aucun lieu ou région associé à ce personnage.</p>';
        }

        textView.innerHTML = html;
        console.log('✅ [renderLieuxRegionsTabRead] Rendu terminé');
    }

    navigateToLocation(locationId) {
        console.log('🔗 [navigateToLocation] Navigation vers lieu ID:', locationId);
        
        const location = window.locationsData?.locations?.find(loc => String(loc.id) === String(locationId));
        if (!location) {
            console.warn('⚠️ Lieu non trouvé avec ID:', locationId);
            return;
        }
    ...
    navigateToRegion(regionId) {
        console.log('🔗 [navigateToRegion] Navigation vers région ID:', regionId);
        
        const region = window.regionsData?.regions?.find(reg => String(reg.id) === String(regionId));
        if (!region) {
            console.warn('⚠️ Région non trouvée avec ID:', regionId);
            return;
        }
